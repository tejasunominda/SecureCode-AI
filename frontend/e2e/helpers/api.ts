...
export async function publishQuestion(request: APIRequestContext, token: string, questionId: string) {
  // Backend requires: submit for review → approve → publish
  // Try each step and ignore INVALID_TRANSITION errors (question may already be in a later state)
  const submitRes = await request.put(`${ASSESSMENT_BASE}/api/v1/assessment/questions/${questionId}/submit-review`, {
    headers: { 'Authorization': `Bearer ${token}` },
  });
  if (!submitRes.ok()) {
    const body = await submitRes.text();
    if (!body.includes('INVALID_TRANSITION')) {
      throw new Error(`Submit for review failed: ${submitRes.status()} ${body}`);
    }
  }

  const approveRes = await request.put(`${ASSESSMENT_BASE}/api/v1/assessment/questions/${questionId}/approve`, {
    headers: { 'Authorization': `Bearer ${token}` },
  });
  if (!approveRes.ok()) {
    const body = await approveRes.text();
    if (!body.includes('INVALID_TRANSITION')) {
      throw new Error(`Approve question failed: ${approveRes.status()} ${body}`);
    }
  }
...
export async function submitQuestionForReview(request: APIRequestContext, token: string, questionId: string) {
  const res = await request.put(`${ASSESSMENT_BASE}/api/v1/assessment/questions/${questionId}/submit-review`, {
    headers: { 'Authorization': `Bearer ${token}` },
  });
  if (!res.ok()) {
    throw new Error(`Submit for review failed: ${res.status()} ${await res.text()}`);
  }
  return res.json();
}

export async function approveQuestion(request: APIRequestContext, token: string, questionId: string) {
  const res = await request.put(`${ASSESSMENT_BASE}/api/v1/assessment/questions/${questionId}/approve`, {
    headers: { 'Authorization': `Bearer ${token}` },
  });
  if (!res.ok()) {
    throw new Error(`Approve question failed: ${res.status()} ${await res.text()}`);
  }
  return res.json();
}

export async function rejectQuestion(request: APIRequestContext, token: string, questionId: string) {
  const res = await request.put(`${ASSESSMENT_BASE}/api/v1/assessment/questions/${questionId}/reject`, {
    headers: { 'Authorization': `Bearer ${token}` },
  });
  if (!res.ok()) {
    throw new Error(`Reject question failed: ${res.status()} ${await res.text()}`);
  }
  return res.json();
}

export async function autoSave(request: APIRequestContext, token: string, sessionId: string, data: { currentSection: string; currentQuestionIndex: number; code: string; language: string; answers: Record<string, string> }) {
  const res = await request.post(`${ASSESSMENT_BASE}/api/v1/assessment/sessions/${sessionId}/autosave`, {
    data,
    headers: {
      'Authorization': `Bearer ${token}`,
      'Content-Type': 'application/json',
  ...
  if (!res.ok()) {
    throw new Error(`Auto-save failed: ${res.status()} ${await res.text()}`);
  }
  return res.json();
}

export async function getAutoSave(request: APIRequestContext, token: string, sessionId: string) {
  const res = await request.get(`${ASSESSMENT_BASE}/api/v1/assessment/sessions/${sessionId}/autosave`, {
    headers: { 'Authorization': `Bearer ${token}` },
  });
  if (!res.ok()) {
    throw new Error(`Get auto-save failed: ${res.status()} ${await res.text()}`);
  }
  return res.json();
}
